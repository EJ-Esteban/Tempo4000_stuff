...
int registerComm(usciConfig *conf) {
	if (devIndex >= MAX_DEVS)
		return -1; // Check device list not full
...
void confUCA0(unsigned int commID) {
	...
	if (devConf[UCA0_INDEX] == commID)
		return; // Check if device is already configured
	enter_critical(status);
	// Perform config in critical section
	...
	spiA0RxSize = 0;
#endif //USE_UCA0_SPI
	UCA0_IO_CONF(dev[commID]->rAddr & ADDR_MASK);
	// Port set up
...
int uartA0Write(unsigned char *data, unsigned int len, unsigned int commID) {
	if (usciStat[UCA0_INDEX] != OPEN)
		return -1; // Check that the USCI is available
...
#endif // USE_UCA0_SPI
		if (UCA0STAT & UCRXERR)
			dummy = UCA0RXBUF; // RX ERROR: Do a dummy read to clear interrupt flag
...
int i2cB0Write(i2cPacket* packet) {
	...
	// Start of TX
	usciStat[UCB0_INDEX] = TX;
	UCB1CTL0 |= UCTR + UCTXSTT; // Generate start condition
...
int i2cB0Read(i2cPacket* packet) {
	...
	ucb0RxPtr = packet->data; // THIS IS OPTIONAL (DUNNO IF WE WANT TO REDIRECT THIS WRITEBACK)

	// Start of RX
	usciStat[UCB0_INDEX] = RX;

	UCB0CTL1 |= UCTR + UCTXSTT; // Generate start condition
	devConf[UCB0_INDEX] = 0; // Clear config
...
int i2cB0SlavePresent(unsigned int commID) {
	...
	if (usciStat[UCB1_INDEX] != OPEN)
		return -1; // Check that USCI is available
...
unsigned int i2cb1RegAddr = 0xFF; ///< Register address storage for I2C operation
...
#endif //USE_UCB1_I2C
	UCB1_IO_CONF(dev[commID]->rAddr & ADDR_MASK);
	// Port set up
...
__interrupt void usciB1Isr(void) {
	...
	case I2CIV_TX_INT: // TX flag
		if (usciStat[UCB1_INDEX] == TX) {
			if (i2cb1RegAddr != 0xFF) { // Are we writing the first byte (register address)?
				UCB1TXBUF = i2cb1RegAddr; // Write the register address
				i2cb1RegAddr = 0xFF; // Zero the register address to indicate transferred
